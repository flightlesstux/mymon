...
panels = [
    ...
    timeseries("EU ${fuel}, selected countries", 0, 13, 12, 9, EU_TREND, unit="currencyEUR", decimals=3, colors=CAT, fill=0),
    timeseries("EU average, petrol 95 vs diesel", 12, 13, 12, 9, EU_AVG, unit="currencyEUR", decimals=3, colors=[CAT[3], CAT[0]], fill=0),
    ...
    timeseries("Istanbul pump prices, TRY/L", 8, 22, 8, 9, TR_TREND, unit="currencyTRY", decimals=2, colors=[CAT[3], CAT[0], CAT[2]], fill=0),
    timeseries("Istanbul pump prices in EUR/L (TCMB rate)", 16, 22, 8, 9, TR_IN_EUR, unit="currencyEUR", decimals=3, colors=[CAT[3], CAT[0], CAT[2]], fill=0,
               description="Comparable with the EU map above."),
    timeseries("United States retail gasoline and diesel, USD/gal (EIA)", 0, 31, 12, 8, US_FUEL, unit="currencyUSD", decimals=3, colors=[CAT[3], CAT[0]], fill=0,
    ...
    timeseries("Natural gas — Europe, US, Japan LNG (World Bank Pink Sheet, USD/MMBtu)", 12, 31, 12, 8,
               pink(["natural_gas_europe", "natural_gas_us", "liquefied_natural_gas_japan"]), unit="currencyUSD", decimals=2, colors=[CAT[0], CAT[1], CAT[2]], fill=0),

    timeseries("Energy — crude oil average, coal, natural gas index (Pink Sheet)", 0, 39, 12, 9,
               pink(["crude_oil_average", "coal_australian", "natural_gas_index"]), decimals=1, colors=[CAT[2], CAT[7], CAT[0]], fill=0,
    ...
    timeseries("Food staples indexed to 100 at range start — wheat, rice, maize, sugar, coffee, cocoa", 12, 39, 12, 9,
               pink_indexed(["wheat_us_hrw", "rice_thai_5", "maize", "sugar_world", "coffee_arabica", "cocoa"]), decimals=0, colors=CAT, fill=0, legend="right"),
    timeseries("Metals indexed to 100 — gold, silver, copper, aluminum, nickel, iron ore", 0, 48, 12, 9,
               pink_indexed(["gold", "silver", "copper", "aluminum", "nickel", "iron_ore_cfr_spot"]), decimals=0, colors=CAT, fill=0, legend="right"),
    timeseries("Fertilizers — urea, DAP, potassium chloride (USD/mt)", 12, 48, 12, 9,
               pink(["urea", "dap", "potassium_chloride"]), unit="currencyUSD", decimals=0, colors=[CAT[2], CAT[3], CAT[6]], fill=0),
    ...
    timeseries("FAO food price indices (2014-16 = 100)", 0, 68, 12, 9, FAO, decimals=0, colors=CAT, fill=0, legend="right"),
    timeseries("Big Mac price in USD, selected countries (The Economist)", 12, 68, 12, 9, BIGMAC_TREND, unit="currencyUSD", decimals=2, colors=CAT, fill=0, legend="right"),
    ...
    timeseries("Inflation, selected countries", 8, 90, 16, 10, INFLATION_TREND, unit="percent", decimals=1, colors=CAT, fill=0, legend="right"),

    timeseries("Household electricity, EUR/kWh incl. taxes (Eurostat, semi-annual)", 0, 100, 8, 9, ELEC, decimals=3, colors=CAT, fill=0),
    timeseries("Household gas, EUR/kWh incl. taxes (Eurostat)", 8, 100, 8, 9, GAS, decimals=3, colors=CAT, fill=0),
    barchart("Household electricity, latest, EUR/kWh", 16, 100, 8, 9, ELEC_LATEST, decimals=3, color=CAT[3]),
    timeseries("Central bank policy rates (BIS / FRED)", 0, 109, 24, 9, POLICY, unit="percent", decimals=2, colors=CAT, fill=0, legend="right"),
]
