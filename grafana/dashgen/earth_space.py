...
panels = [
    ...
    timeseries("Quakes per day by magnitude band (USGS)", 0, 18, 12, 8, QUAKES_PER_DAY, decimals=0, stack=True, fill=70, lw=1,
               colors=[CAT[7], CAT[1], CAT[3]]),
    table("Biggest in range", 12, 18, 12, 8, BIGGEST, overrides=mag_overrides),
    timeseries("Magnitude of every M4+ event", 0, 26, 24, 7, MAG_SCATTER, decimals=1, colors=[CAT[1]], fill=0, points=True, lw=0,
               description="Each dot is one earthquake."),

    timeseries("Planetary Kp index (3-hour)", 0, 33, 8, 8, KP, decimals=1, colors=[CAT[6]], fill=40, min_=0, max_=9,
               description="Kp 5+ = geomagnetic storm, aurora possible at mid-latitudes."),
    timeseries("Solar wind speed", 8, 33, 8, 8, WIND, unit="velocitykms", decimals=0, colors=[CAT[3]]),
    timeseries("Bz and proton density", 16, 33, 8, 8, BZ, decimals=1, colors=[CAT[0], CAT[4]], fill=0,
    ...
    timeseries("ISS altitude", 12, 41, 6, 10, ISS_ALT, unit="lengthkm", decimals=1, colors=[CAT[6]], fill=0),
    timeseries("ISS velocity", 18, 41, 6, 10, ISS_VEL, unit="velocitykmh", decimals=0, colors=[CAT[2]], fill=0),

    timeseries("Atmospheric CO₂ at Mauna Loa (NOAA)", 0, 51, 16, 9, CO2, unit="ppm", decimals=1, colors=[CAT[7], CAT[3]], fill=0),
    timeseries("CO₂ year-over-year growth", 16, 51, 8, 9, CO2_YOY, unit="ppm", decimals=2, colors=[CAT[1]], fill=20),

    timeseries("Bitcoin network hashrate", 0, 60, 8, 8, BTC_HASH, decimals=0, colors=[CAT[3]], fill=20, description="EH/s (exahashes per second)."),
    timeseries("Recommended fees, sat/vB", 8, 60, 8, 8, BTC_FEES, decimals=0, colors=[CAT[7], CAT[1], CAT[3], CAT[2]], fill=0),
    timeseries("Mempool unconfirmed transactions", 16, 60, 8, 8, BTC_MEMPOOL, unit="short", decimals=0, colors=[CAT[0]], fill=30),
]
