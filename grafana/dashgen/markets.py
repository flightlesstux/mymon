...
panels = [
    ...
    timeseries("Turkish lira — USD, EUR, GBP (TCMB selling)", 0, 4, 12, 9, FX_TRY, decimals=2, colors=[CAT[0], CAT[1], CAT[2]]),
    timeseries("Majors vs USD", 12, 4, 12, 9, FX_MAJORS, decimals=4, colors=[CAT[0], CAT[2]]),
    timeseries("USD vs emerging currencies, indexed to 100 at range start", 0, 13, 12, 9, FX_INDEXED, decimals=1,
               colors=CAT, fill=0, description="Rising line = currency weakening against the dollar."),
    timeseries("USD/TRY, USD/BRL, USD/MXN, USD/ZAR, USD/INR", 12, 13, 12, 9, FX_EM, decimals=2, colors=CAT, fill=0),

    timeseries("Crypto, indexed to 100 at range start", 0, 22, 12, 9, CRYPTO_INDEXED, decimals=0, colors=[CAT[3], CAT[6], CAT[4], CAT[1], CAT[0]], fill=0),
    timeseries("BTC intraday (1-minute ticks)", 12, 22, 12, 9, crypto_line("BTC"), unit="currencyUSD", decimals=0, colors=[CAT[3]]),
    ...
    timeseries("BTC priced in gold ounces", 8, 31, 8, 8, BTC_GOLD, decimals=1, colors=[CAT[3]], fill=15),
    timeseries("Tokenised gold (PAXG) vs spot gold", 16, 31, 8, 8, PAXG_VS_GOLD, unit="currencyUSD", decimals=0, colors=[CAT[3], CAT[0]], fill=0),

    timeseries("Oil — Brent and WTI, USD/bbl", 0, 39, 12, 9, OIL, unit="currencyUSD", decimals=2, colors=[CAT[2], CAT[1]], fill=0),
    timeseries("Gold and silver, USD/oz", 12, 39, 12, 9, METALS, unit="currencyUSD", decimals=2, colors=[CAT[3], CAT[0]], fill=0,
               description="Silver is on the same axis; use the legend to isolate a series."),
    timeseries("Stock indices, indexed to 100 at range start", 0, 48, 24, 10, INDICES_INDEXED, decimals=0, colors=CAT, fill=0, legend="right",
